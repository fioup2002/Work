function CreateJSONObject(e, s) {
  ...
  return (
    e == CMD_LOGIN
      ...
      : e == CMD_REPORT_ORDER
      ? null != s && 0 != s.length && ((t.ref_id = s[0]), (t.pay_info = new Object()), (t.pay_info.bank_code = s[1]), (t.pay_info.acc_code = s[2]), (t.pay_info.pay_time = s[3]), (t.pay_info.pay_amount = s[4]), (t.pay_info.note = s[5]))
      : e == CMD_AUTH_CODE
...
function ParseReponse(e, s, t) {
  ...
  // }
  /*test*/
  if ((0 != gToken.length && "success" != r.status && e != CMD_MODIFY && e != CMD_GET_CLASS_CONTENT && e != CMD_REPORT_ORDER && ((gToken = ""), "undefined" != typeof Storage && localStorage.setItem("token", gToken), console.log(e), console.log(s), console.log(JSON.stringify(r)), alert("登入逾期") || window.location.assign("./index.htm")), e == CMD_GET_CLASS)) {
    ...
    if (r.status == "success") {
      ...
      $(".act_content_detail").html("").hide();
      $("#act_detail_" + index)
        .append(GenerateActivityDetail(index))
        .show();
    }
  } else if (e == CMD_LOGIN) "success" == r.status ? (SetDefaultAlerMenu(), $(".alert_menu").append(GenerateAuthMenu())) : 1 == r.code ? $("#login_error_text").html("登入失敗: 帳號/密碼錯誤") : 2 == r.code ? $("#login_error_text").html("登入失敗: 不存在的帳號") : 3 == r.code ? $("#login_error_text").html("登入失敗: 帳號已在別的裝置上登入") : 4 == r.code && $("#login_error_text").html("登入失敗: email 寄送異常");
    ...
    e == CMD_FORGET
      ...
      : e == CMD_CHECK_TOKEN
      ? "success" == r.status
        ? ((gAccount = r.user_id), UpdateLoginInformation(), gIsCheckToken && (gCurrentPage == PAGE_INDEX && SendCommand(CMD_GET_CLASS, CreateJSONObject(CMD_GET_CLASS, null)), SendCommand(CMD_GET_USER_INFO, CreateJSONObject(CMD_GET_USER_INFO, new Array()))))
      ...
      : e == CMD_GET_USER_INFO
      ? "success" == r.status && ((gName = r.userData.name), (gEmail = r.userData.email), (gPhone = r.userData.phone), $("#user_name").val(gName), $("#user_email").val(gEmail), $("#user_mobile").val(gPhone), null != r.userData.isAdmin && 1 == r.userData.isAdmin ? ((gAdmin = !0), $("#admin_page").css("visibility", "visible")) : ((gAdmin = !1), $("#admin_page").css("visibility", "hidden")))
      : e == CMD_MODIFY
      ? "success" == r.status
      ...
      : e == CMD_BUY_CLASS
      ? "success" == r.status
        ? ($("#buy_error_text").html("購買成功。匯款完成後，請到會員中心做匯款回報").css("color", "#000"), $("#buy_button").html("返回首頁"), (gIsBuy = !1), (gShoppingCarList = new Array()), UpdateUnboughtClass())
      ...
      ? "success" == r.status
        ...
        : alert("email驗證訊息寄送失敗\n請檢查填寫的email格式是否正確\n或者請洽網站管理員")
      : e == CMD_SEND_EMAIL_CODE && ("success" == r.status ? ($("#mail_hint").html("已驗證信箱").css("color", button_ok_color.split(":")[1]), (gIsVerifyMail = !0)) : $("#mail_hint").html("驗證失敗").css("color", button_cancel_color.split(":")[1]));
}
