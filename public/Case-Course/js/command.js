function CreateJSONObject(e, s) {
  ...
  return (
    e == CMD_LOGIN
      ...
      : e == CMD_MODIFY
      ? (0 != s[0].length && (t.prev_pwd = s[0]),
        0 != s[1].length && (t.pwd = s[1]),
        (t.name = s[2]),
        0 != s[3].length && (t.email = s[3]),
        (t.phone = s[4]))
      : e == CMD_GET_CLASS
...
function SendCommand(e, s) {
  ...
  e == CMD_GET_CLASS
    ...
    : e == CMD_REGISTER
    ? ((t = server + user_api + indexphp + "/user/account/create"),
      console.log(s))
    : e == CMD_FORGET
    ? ((t = server + user_api + indexphp + "/user/account/reset"),
      (a = "GET"),
      (n = "json="))
    : e == CMD_CHECK_TOKEN
    ? ((t = server + user_api + indexphp + "/user/login/verify"),
      (a = "GET"),
      (s = ""))
    : e == CMD_GET_USER_INFO
    ? ((t = server + user_api + indexphp + "/user/account/fetch/info"),
      (a = "GET"),
      (s = ""))
    : e == CMD_MODIFY
    ...
    : e == CMD_GET_ORDER
    ? ((t = server + order_api + indexphp + "/order/search"),
      (a = "GET"),
      (n = "json="))
    : e == CMD_REPORT_ORDER
    ...
    : e == CMD_LOGOUT
    ? ((t = server + user_api + indexphp + "/user/logout"),
      (s = ""),
      (a = "GET"))
    : e == CMD_RE_SEND_EMAIL
    ? (t = server + user_api + indexphp + "/user/email/send/verify")
    : e == CMD_SEND_EMAIL_CODE &&
      (t = server + user_api + indexphp + "/user/email/verify"),
...
function ParseReponse(e, s, t) {
  ...
  if (
    ...
    if ("success" == r.status) {
      var n = new Array();
      n.push($.parseJSON(s).user_id),
        n.push($.parseJSON(s).user_pwd),
        SendCommand(CMD_LOGIN, CreateJSONObject(CMD_LOGIN, n)),
        HideAlertMenu();
    } else $("#register_error_text").html("註冊失敗");
  else
    e == CMD_FORGET
      ? "success" == r.status
        ? $("#froget_password_hint")
            .html("請確認信箱是否收到")
            .css("color", "#000")
        ...
        ? ((gAccount = r.user_id),
          ...
          gIsCheckToken &&
            (gCurrentPage == PAGE_INDEX &&
              SendCommand(CMD_GET_CLASS, CreateJSONObject(CMD_GET_CLASS, null)),
            SendCommand(
              CMD_GET_USER_INFO,
              CreateJSONObject(CMD_GET_USER_INFO, new Array())
            )))
        : ((gToken = ""),
          gIsCheckToken
            ? gCurrentPage == PAGE_INDEX &&
      ...
      ? "success" == r.status &&
        ...
        null != r.userData.isAdmin && 1 == r.userData.isAdmin
          ? ((gAdmin = !0), $("#admin_page").css("visibility", "visible"))
          : ((gAdmin = !1), $("#admin_page").css("visibility", "hidden")))
      : e == CMD_MODIFY
      ? "success" == r.status
      ...
      ? "success" == r.status &&
        null != r.courseData &&
        ((gClass_content = r.courseData), UpdateClassContent())
      : e == CMD_BUY_CLASS
      ? "success" == r.status
        ? ($("#buy_error_text")
            .html("購買成功。匯款完成後，請到會員中心做匯款回報")
            .css("color", "#000"),
      ...
      : e == CMD_SEND_EMAIL_CODE &&
        ("success" == r.status
          ? ($("#mail_hint")
              .html("已驗證信箱")
              .css("color", button_ok_color.split(":")[1]),
            (gIsVerifyMail = !0))
          : $("#mail_hint")
              .html("驗證失敗")
              .css("color", button_cancel_color.split(":")[1]));
}
function AddRealClass(e) {
  for (var s = !1, t = 0; t < gReal_classes.length; t++)
    gReal_classes[t].id == e.id && (s = !0);
  s || gReal_classes.push(e);
}
function AddBoughtClass(e) {
  for (var s = !1, t = 0; t < gBought_classes.length; t++)
    gBought_classes[t].id == e.id && (s = !0);
  s || gBought_classes.push(e);
}
function AddUnboughtClass(e) {
  for (var s = !1, t = 0; t < gClasses.length; t++)
    gClasses[t].id == e.id && (s = !0);
  s || gClasses.push(e);
}
